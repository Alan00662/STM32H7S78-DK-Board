...
/* USER CODE BEGIN 1 */
void uart7_send_byte(uint8_t Data)
{
	HAL_UART_Transmit(&huart7, (uint8_t *)&Data,1, 100);

}

void uart7_send_buf(uint8_t *buf, uint8_t len)
{
    uint8_t i;
    for (i = 0; i < len; i++)
    {
        uart7_send_byte(*(buf + i));
    }
}
...
void uart7_send_string(char *str)
{
    ...
    {
        uart7_send_byte(*(str + i));
    }
}

void debug_tx7(char *fmt, ...)
{
    int len;
    char buf[200];
